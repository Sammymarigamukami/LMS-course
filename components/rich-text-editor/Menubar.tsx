...
export function Menubar({ editor }: MenubarProps) {
    ...
    // Force a re-render whenever the editor's state or selection changes
    // so that editor.isActive(...) updates correctly
    const [,setUpdate] = useState(0)
    
    useEffect(() => {
        if (!editor) 
            return
        const update = () => setUpdate((x) => x + 1)
    ...
    return(
        ...
        className="border border-input border-t-0 border-x-0 rounded-t-lg p-2 bg-card flex flex-wrap gap-1 items-center">
            <TooltipProvider>
                ...
                <div className="w-px bg-border mx-2 flex">
